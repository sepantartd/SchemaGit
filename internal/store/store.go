...
func (s *Store) LoadMembersByProject(pid int64) []ProjectMember {
    ...
    for rows.Next() {
        var m ProjectMember
        rows.Scan(&m.ID, &m.ProjectID, &m.Email, &m.Role, &m.InvitedAt)
        out = append(out, m)
    }
...
func (s *Store) LoadWebhooksByProject(pid int64) []ProjectWebhook {
    ...
    for rows.Next() {
        ...
        rows.Scan(&w.ID, &w.ProjectID, &w.URL, &w.Secret, &w.Type, &w.CreatedAt)
        out = append(out, w)
    }
...
func (s *Store) LoadJobHistory() []Job {
    ...
    for rows.Next() {
        ...
        rows.Scan(&id, &typ, &payloadRaw, &status, &created, &updated, &attempts, &lastErr)
